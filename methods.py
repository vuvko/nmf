...
def my_als(V, W, H, post='', cfg=config.default_config()):
    eps = cfg['eps']
    T = H.shape[0]
    W0 = W
    H0 = H
    for k in range(T):
        W0[:, k] = 0
        cw = (dot(V - dot(W0, H0), H0[k, :].T))
        cw = cw - sum(cw * W0[:, k])
        W0[:, k] =  cw / (sum(H0[k, :] ** 2) + eps)
        W0[W0 < eps] = 0
        H0[k, :] = 0
        ch = (dot((V - dot(W0, H0)).T, W0[:, k]).T)
        ch = ch - sum(ch * H0[k, :])
        H0[k, :] = ch / (sum(W0[:, k] ** 2, 0) + eps)
        H0[H0 < eps] = 0
    return W, H
...
def mult(V, W, H, post='', cfg=config.default_config()):
    ...
    eps = cfg['eps']
    W0 = W * dot(V, H.T) / (dot(dot(W, H), H.T) + eps)
    H = H * dot(W.T, V) / (dot(dot(W.T, W), H) + eps)
    return (W0, H)
